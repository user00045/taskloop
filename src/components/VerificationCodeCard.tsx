...
const VerificationCodeCard: React.FC<VerificationCodeCardProps> = ({
  ...
  const renderStatus = () => {
    if (isVerified && isPartnerVerified) {
      return <span className="text-green-600 flex items-center"><Check className="h-4 w-4 mr-1" /> Task completion verified</span>;
    } else if (isVerified) {
      return <span className="text-yellow-600">Waiting for {isDoer ? "creator" : "doer"} confirmation</span>;
    } else if (isPartnerVerified) {
      return <span className="text-yellow-600">{isDoer ? "Creator" : "Doer"} has verified. Please verify to complete.</span>;
    } else {
    ...
    <Card className="w-full mb-4">
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div>
            <p className="text-sm font-medium mb-1">Verify {isDoer ? "Creator" : "Doer"}'s Code:</p>
            <div className="flex">
              <Input
